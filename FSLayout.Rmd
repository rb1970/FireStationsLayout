...
This code chunk imports the functions defined in the R script _Functions2FSLayout.R_:
...
```{r}
areas<-unlist(lapply(df.region, areaPolygon))*10^-6 #default unit of distance is meter, convert to km2
...
```{r}
ppp.all<-superimpose(H$points)
d.year<-density.ppp(ppp.all,edge = TRUE,diggle = TRUE)/9
plot(d.year,main=" ",cex.main=1.5, ribbon=T,addcontour=F,clipwin=win[[1]],box=F)
```
...
par(mar=c(4,1,2,1))
par(pty="s")
Ki1<-Kinhom(ppp.list[[1]],correction = "border",edge=T,diggle=T)
plot(Ki1,main=" ")
for (i in 2:9) {
 Ki <- Kinhom(ppp.list[[i]],correction = "border",edge=T,diggle=T)
plot(Ki,main=" ",add=T)
}

Li1<-Linhom(ppp.list[[1]],correction = "border",edge=T,diggle=T)
plot(Li1,main=" ")
for (i in 2:9) {
 Li <- Linhom(ppp.list[[i]],correction = "border",edge=T,diggle=T)
plot(Li,main=" ",add=T)
}
```
...
model.list <- list(
fit=mppm(points ~ 1, H),
fit0=mppm(points ~ 1+offset(log(image)), H),
fit1=mppm(points ~ x+y+x:y+offset(log(image)), H),
...
listm<-lapply(model.list,function(x) round(summary(x)$Fit$FIT$coeff,3))
listm

#AIC
llik=unlist(lapply(model.list, logLik));llik
aic=unlist(lapply(model.list, AIC));aic-min(aic)
sumbest<-summary(fit)$Fit$FIT$coeff
# Summary results for the best model
sumbest 
...
```{r,fig.width=15}
persp(pred,theta=-30,phi=40,ticktype="detailed",zlab="Density")
```

## Simulation

Given the point process model fitted to data, the following code generates random simulations of the model.
...
The next chunk creates a point pattern to represent the FS locations in the two-dimensional plane and estimates the respective density.
...
plot(pred,col=heat.colors(300,rev=T),main="(A)",ribbon=F,box=F)

plot(dens,col = terrain.colors(300,rev=T),addcontour=F,box=F,ribbon=F,main="(B)");points(maxXY,pch=8,cex=1,col=1);plot(win[[1]],add=T)

plot(ppp.list[[1]],type="n",main="(C)");points(alloc$long,alloc$lat,type="p",cex=0.5,pch=16,col=2,add=T);segments(alloc$long,alloc$lat,alloc$longOPT,alloc$latOPT,col=2,lwd=0.3);points(alloc$longOPT,alloc$latOPT,col=1,cex=1,pch=8);plot(final,add=T,lwd=1)

```

+ Actual vs. new locations:
...
points(coordOPT$lon,coordOPT$lat,pch=8,cex=0.7)
